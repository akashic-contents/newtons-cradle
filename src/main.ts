...
/** 鉄球のパラメータ */
const ballParameter: BallParameterObject = {
	appear: {
 ...
 */
function createCircle(scene: g.Scene, parameter: BallParameterObject): box2d.EBody | null {
	// 画像をまとめる空のエンティティを生成
 ...
 */
function addMouseJoint(ebody: box2d.EBody): void {
	// オブジェクトを触れるようにする
	ebody.entity.touchable = true;

	// タッチされている座標（Box2D上の絶対座標）
	let anchor: box2d.Box2DWeb.Common.Math.b2Vec2;

	/** 鉄球とマウスを結びつけるジョイント */
	let mouseJoint: box2d.Box2DWeb.Dynamics.Joints.b2MouseJoint = null;
	...
	ebody.entity.onPointDown.add((event) => {
		...
		// マウスジョイントを生成
		mouseJoint = physics.world.CreateJoint(mouseJointDef) as box2d.Box2DWeb.Dynamics.Joints.b2MouseJoint;
	});
	// タッチ中の座標が移動したときの処理
 ...
 */
function addDistanceJoint(
	ebody: box2d.EBody,
	fulcrum: box2d.Box2DWeb.Common.Math.b2Vec2,
	contact: box2d.Box2DWeb.Common.Math.b2Vec2
): void {
